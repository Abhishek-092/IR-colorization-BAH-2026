...
from inference import refiner

# Track-2 realism refiner. ON by default (SD-Turbo photorealistic pass). It
# degrades gracefully — if diffusers isn't installed or the model can't load,
# refine() returns the input unchanged, so the dashboard still works. Set
# SUTRAM_REFINER=0 to force it off.
...
class SutramModel:
    ...
    def _sync(self):
        """Block until queued GPU or MPS operations finish."""
    ...
    @staticmethod
    def _denorm_sr(sr):
        # Denormalize super-resolved output to Kelvin
        return torch.clamp(sr * (TB_MAX - TB_MIN) + TB_MIN, TB_MIN, TB_MAX)

    @torch.no_grad()
    def infer(self, lr_norm):
        """
        Run forward pass for model inference.
        `lr_norm` is a float32 tensor (1,1,H,W) of normalized brightness temperature in [0, 1].
        Returns a dictionary of predicted brightness temperature, RGB synthesis, and uncertainty maps.
        """
        ...
        # Annealed mixture mean expectation for output display
        T_ANNEAL = 0.4
        pi_sharp = F.softmax(logit_w / T_ANNEAL, dim=1)
        rgb_expect = (pi_sharp.unsqueeze(2) * means).sum(1)   # (B,3,H,W)
        self._sync()
...
def greyscale_thermal(arr):
    """Percentile-stretch thermal values into a 0-255 grayscale single-channel image."""
...
def normalize_rgb(rgb_raw):
    """Normalize raw RGB predictions to a viewable uint8 image (0-255 range)."""
...
def display_stretch(rgb_u8):
    """Apply global luminance affine gain to RGB images for visualization."""
...
# All GPU work (model inference + diffusion refiner) is serialized through this
# lock. Flask serves requests on multiple threads, but MPS/Metal (and CUDA
# contexts) are not safe to drive concurrently — overlapping inferences caused
# an IOGPUMetalCommandBuffer assertion crash. One request on the GPU at a time.
_INFER_LOCK = threading.Lock()


def run_inference_payload(lr_np, meta):
    """Shared inference + rendering for both upload and demo-scene requests.
    Serialized on the GPU lock to keep MPS/CUDA single-threaded."""
    with _INFER_LOCK:
        return _run_inference_payload(lr_np, meta)


def _run_inference_payload(lr_np, meta):
    ...
    # Optional Track-2 realism refiner (pretrained SD-Turbo, low-strength
    # img2img). Off unless SUTRAM_REFINER=1; on any failure it returns the
    # input unchanged, so the core pipeline is never broken by it. Refines the
    # fused reconstruction (the shown "ground reconstruction") — the material
    # map and grids keep using the un-refined physics prediction.
    if REFINER_ON:
