...
class RCAB(nn.Module):
    """Residual Channel-Attention Block — the workhorse where detail is formed."""
...
class SRHead(nn.Module):
    """
    Multi-scale thermal Super-Resolution head (~0.6M params). Upsamples the
    200m backbone features to a 100m brightness-temperature field (2x).

    This is the improved architecture: it fuses ALL backbone scales f1-f4 (deep
    features carry the context needed to invert the sensor's box-average blur),
    refines with residual channel-attention blocks before AND after the 2x
    PixelShuffle upsample, and keeps a bilinear global-residual path so the
    network only has to learn the high-frequency restoration on top of a clean
    upscale. It scores ~+1.5 dB over bicubic and ~+10 dB over the old 28k-param
    f1-only head (which scored *below* bicubic). Same call signature as before —
    sr_head(features, lr_tir) — so it is a drop-in across the whole pipeline.
    """
    def __init__(self, in_channels=32, out_channels=1, ch=64):
        super().__init__()
        # Project the deeper context features to a common width.
        ...
        # Fuse f1 + 3 context maps + the raw LR input at the f1 resolution.
        ...
        # High-resolution refinement trunk (where fine detail is actually made).
