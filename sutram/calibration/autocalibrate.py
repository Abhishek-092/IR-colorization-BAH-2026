"""
Auto-calibration module for converting input thermal or optical rasters into calibrated
physical units (brightness temperature in Kelvin) or normalized display ranges.

Used across dataset loading, inference pipelines, and web serving to maintain consistency.
Functions operate on numpy arrays and detect input encodings at runtime; model components
consume normalized [0, 1] brightness-temperature tensors.

Supported thermal encodings:
  * 8-bit thermal (<= 255)            -> scaled to approximate brightness temperature range.
  * L1 Band-10 DN (uint16)            -> Planck inversion (dn_to_brightness_temp).
  * L2 ST_B10 surface temp (uint16)    -> standard L2 scaling (0.00341802 * DN + 149).

RGB encodings:
  * 8-bit RGB (<= 255)                -> passthrough (0-255 range).
  * Synthetic reflectance (0..10000)  -> mapped to 0-255 range.
  * L2 SR reflectance (uint16)         -> L2 reflectance transform stretched to 0-255.
"""
...
def thermal_is_physical(arr):
    """Returns True if input thermal array contains radiometric data (L1/L2), False for 8-bit image data."""
